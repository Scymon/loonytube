...
function VideoThumb({ video }: { video: SavedVideo }) {
  return (
...
export default function SavedSection({ groups = [] }: Props) {
  return (
    <section>
      <div className="mb-4 flex items-center justify-between">
        <h2 className="flex items-center gap-2 text-base font-bold text-foam">
          <span>&#9670;&#65039;</span> Saved / Watch Later
        </h2>
        <Link href="/saved" className="text-xs text-mist hover:text-foam transition">
          See All &rsaquo;
        </Link>
      </div>

      {groups.length === 0 ? (
        <p className="text-sm text-mist">Videos you save will appear here.</p>
      ) : (
